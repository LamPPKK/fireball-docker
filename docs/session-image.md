...
Workflow run [`32472301428`](https://github.com/LamPPKK/fireball-docker/actions/runs/32472301428) at commit `03ad91d11c18a20a2f95cb677eeaa56af366817d` is the first **PASS** that also requires public failed-session observation after a real pipeline crash. The amd64 and arm64 jobs completed in 4 minutes 21 seconds and 4 minutes 17 seconds. Each native runner killed the WPE/GStreamer runtime, observed a non-zero stopped container with restart count zero, required `GET /orchestrator/v1/sessions/:id` to return the same identity with `phase=failed` and the bounded failure reason, proved a pre-crash signaling ticket was already revoked before Burn, removed all residue, and created a healthy replacement. Every other source gate, including Direct and two consecutive relay-only TURN media/control handshakes, also passed. This is source-revision evidence and does not replace the no-rebuild OCI candidate lane.
