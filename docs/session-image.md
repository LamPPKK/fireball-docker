...
The `0.1.0-dev.1` session image is an E1 engineering candidate. Its source contract, bootstrap boundary, Docker lifecycle, two-architecture build/start/smoke gate, real two-tenant infrastructure gate, and source-revision H.264/Opus/control gate are implemented and tested. It is not a release until browser-state isolation, real TURN, and all gates are repeated against and promote the same immutable digest.
