...
This is an engineering candidate, not a promoted release. The normal CI validates source, contracts, authentication behavior, deployment-adapter configuration, and image provenance. [`session-image` run 32454792346](https://github.com/LamPPKK/fireball-docker/actions/runs/32454792346) passed the complete source-revision gate on native `linux/amd64` and `linux/arm64` runners at commit `2e3514b79b53e0b8a689b727ca54bbd490b0382d`. Both jobs built and inspected the image, started its sandboxed WPE runtime, passed the real two-tenant infrastructure gate, and completed two authenticated browser connections carrying H.264 video, Opus audio, and control DataChannel responses. Reconnect used a newly issued one-use ticket; burn then closed signaling, revoked a remaining ticket, and left no managed container or network. The gate proves real media negotiation and decoded frames, but it is not a bitrate/latency/thermal benchmark and does not yet prove browser cookie/storage isolation, real TURN traversal, or exact-digest promotion. See [the session-image architecture and promotion gates](docs/session-image.md).
