...
This is an engineering candidate, not a promoted release. The normal CI validates source, contracts, authentication behavior, deployment-adapter configuration, and image provenance. [`session-image` run 32449711590](https://github.com/LamPPKK/fireball-docker/actions/runs/32449711590) passed build, start, health, authentication, reconnect, cleanup, and the real two-tenant infrastructure gate on native `linux/amd64` and `linux/arm64` runners at commit `e713d2b5d72e559aea2251544f055374d4187c39`. The gate proves API ownership denial, signaling-ticket mapping and revocation, distinct bootstrap secrets, container/PID/network/mount/tmpfs separation, peer-network denial, quotas, confinement, and burn residue cleanup. It does not yet prove browser cookie/storage isolation, end-to-end H.264 performance, WebRTC media negotiation, real TURN traversal, or exact-digest promotion. See [the session-image architecture and promotion gates](docs/session-image.md).
