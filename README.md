...
Fireball's multi-tenant remote-browser orchestrator. The repository has completed the bounded multi-tab E1 candidate gate: production OIDC authentication, authenticated WebSocket signaling, one isolated container per tenant session, one to four tabs inside that container, WPE/GStreamer media and control, transactional Docker Engine lifecycle, fail-closed restart reconciliation, exact-digest multi-architecture promotion, and a real Nginx TLS/WebSocket deployment check.
...
- One container, Docker network, tmpfs namespace, quota, and credential set per browser session. One to four tabs may share that tenant's WebKit context; no WebKit context is shared between tenants.
...
- The session-image candidate uses a native C++20 GStreamer runtime with one `wpesrc` per tab, video/audio selectors feeding a single H.264/Opus WebRTC stream, GStreamer navigation over the control DataChannel, no public STUN default, and a separate one-controller bootstrap proxy. Switching tabs changes the selected streams without replacing the tenant container. Each tenant keeps Docker's private PID namespace; a fail-closed bubblewrap argument wrapper reuses that boundary while retaining WebKit's mount/user/network/IPC/UTS and seccomp sandbox layers. Debian and `gst-plugins-rs` provenance are immutable inputs.
...
- Authenticated session reads, signaling credential use, and tab mutations re-check the exact instance-owned Docker container. A stopped or missing runtime atomically moves the session to `failed`, revokes every pending credential and live relay, and remains observable until the owner burns it. A Docker Engine status error returns `503` without falsely declaring the runtime dead.
...
| `GET` | `/orchestrator/v1/sessions/:id/tabs` | Bearer JWT or development token | List the active session's one to four tabs. |
| `POST` | `/orchestrator/v1/sessions/:id/tabs` | Bearer JWT or development token | Create and activate a tab; an omitted URL opens `fireball://home`. |
| `PUT` | `/orchestrator/v1/sessions/:id/tabs/:tabId/active` | Bearer JWT or development token | Select the tab whose video/audio feeds the session stream. |
| `PUT` | `/orchestrator/v1/sessions/:id/tabs/:tabId/navigation` | Bearer JWT or development token | Navigate a tab to a validated HTTP(S) URL. |
| `DELETE` | `/orchestrator/v1/sessions/:id/tabs/:tabId` | Bearer JWT or development token | Delete a tab while retaining at least one tab in the session. |
...
This is an engineering candidate, not a production release. The normal CI validates source, contracts, authentication behavior, deployment-adapter configuration, and image provenance. [`session-image` run 32469946766](https://github.com/LamPPKK/fireball-docker/actions/runs/32469946766) passed the complete source-revision gate on native `linux/amd64` and `linux/arm64` runners at commit `248d97db151a25e9ed535d76325f8093f2096cb4`; the jobs completed in 4 minutes 10 seconds and 4 minutes 5 seconds. Both jobs built and inspected the image, passed the real two-tenant and browser-state gates, kept one container and one native runtime PID through a create/activate/navigate/delete lifecycle of four real WPE tabs, enforced the four-tab maximum and one-tab minimum, and completed Direct plus relay-only TURN media/control. Every browser connection required H.264 video, Opus audio, decoded frames, and a control DataChannel response. Reconnect used newly issued one-use credentials; burn then closed signaling, revoked a remaining ticket, and left no managed container or network. This is functional evidence, not a bitrate, latency, thermal, memory-pressure, or production-capacity benchmark, and the source workflow does not promote an OCI digest.

[`session-image` run 32472301428](https://github.com/LamPPKK/fireball-docker/actions/runs/32472301428) passed the same complete source gate at commit `03ad91d11c18a20a2f95cb677eeaa56af366817d`; amd64 completed in 4 minutes 21 seconds and arm64 in 4 minutes 17 seconds. In both jobs the crash step killed the native WPE/GStreamer runtime, observed non-zero container termination with no Docker restart, required the public session API to transition that exact session to `failed`, rejected its previously issued ticket before Burn, cleaned every managed resource, and created a healthy replacement. Direct H.264/Opus/control and relay-only TURN then passed as well. This run proves the new stopped-runtime observation path against source-built images; it still does not promote an immutable OCI digest.

The manual `session-candidate` workflow implements the immutable promotion lane without rebuilding after QA. Each native runner pushes one commit-scoped platform image, resolves its registry manifest digest, pulls and tests that exact digest through every current image/isolation/browser-state/restart-crash/multi-tab/Nginx-TLS/Direct/TURN gate, emits an SPDX SBOM, and attaches platform provenance plus SBOM attestations. Only after both jobs pass does the merge job create `ghcr.io/lamppkk/fireball-session:candidate-<commit>` from those two digests, validate the raw two-platform OCI index, emit strict candidate evidence, attach index provenance and evidence attestations, and sign the index digest with GitHub OIDC/Cosign. All third-party actions are pinned by commit. [`session-candidate` run 32470322375](https://github.com/LamPPKK/fireball-docker/actions/runs/32470322375) passed this lane at commit `248d97db151a25e9ed535d76325f8093f2096cb4`; the locked platform jobs completed in 6 minutes 15 seconds (`linux/amd64`) and 5 minutes 20 seconds (`linux/arm64`), followed by a 59-second no-rebuild index job. The resulting index is `ghcr.io/lamppkk/fireball-session@sha256:0c092da4f5b67681c19ad8f6d75325be09801d8e884235bb814aab7a31d81bcf`. The tag is a discovery aid; deployment must use this recorded digest identity. See [the session-image architecture and promotion gates](docs/session-image.md).
...
The test suite covers cross-tenant denial, public session redaction, pairing/signaling replay and expiry, exact-origin WebSocket upgrades, dual-hop authentication, frame relay, burn-time socket revocation, session bootstrap isolation, one-controller enforcement, stopped/missing-runtime failure observation and credential revocation, failed cleanup state, real asymmetric JWT signing/verification, Docker isolation options, real WPE cookie/localStorage/service-worker and burn/recreate separation, one-container four-tab lifecycle and bounds, active stream switching, loopback-only signaling publication, read-only TURN secret mounts, strict ICE configuration, injection-safe Nginx rendering, startup health gating, idempotent cleanup, create rollback, restart reconciliation ownership, pipeline-crash containment, aggregate cleanup failure, and concurrent quota reservations.
...
- Run load, memory-pressure, bitrate, latency, and capacity benchmarks before making production sizing claims.
