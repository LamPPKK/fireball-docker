...
This is an engineering candidate, not a promoted release. The normal CI validates source, contracts, authentication behavior, deployment-adapter configuration, and image provenance. [`session-image` run 32458359428](https://github.com/LamPPKK/fireball-docker/actions/runs/32458359428) passed the complete source-revision gate on native `linux/amd64` and `linux/arm64` runners at commit `dac00edaffca1a6f43065ca41f84507f2b5dd28f`. Both jobs built and inspected the image, started its sandboxed WPE runtime, passed the real two-tenant infrastructure gate, and completed two authenticated Direct connections plus two relay-only connections through an ephemeral coturn service. Every browser connection required H.264 video, Opus audio, decoded frames, and a control DataChannel response; the TURN gate additionally required the selected local and remote ICE candidates to both be `relay`. Reconnect used newly issued one-use credentials; burn then closed signaling, revoked a remaining ticket, and left no managed container or network. The gate proves source-revision media/control and TURN behavior, but it is not a bitrate/latency/thermal benchmark and does not yet prove browser cookie/storage isolation or exact-digest promotion. See [the session-image architecture and promotion gates](docs/session-image.md).
...
- Repeat the already-passing WPE, H.264/Opus, control DataChannel, reconnect, burn, and residue gate against that exact immutable digest.
...
- Repeat the real two-tenant gate against the immutable candidate digest, then add browser-level cookie, local-storage, service-worker, and restore isolation tests. The source-revision gate already proves process/network/mount/tmpfs and signaling boundaries.
