...
- Run the protected manual `session-candidate` workflow on `main` and retain its exact OCI index digest, per-platform SBOMs, attestations, signature, and evidence artifact. The lane is implemented but is not counted as evidence until a real GHCR run passes.
